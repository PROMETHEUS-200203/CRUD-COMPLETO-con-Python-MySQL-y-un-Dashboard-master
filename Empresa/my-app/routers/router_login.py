...
def actualizarPerfil():
    if request.method == 'POST':
        if 'conectado' in session:
            print("Sesión activa. Procesando actualización...")  # Depuración

            respuesta = procesar_update_perfil(request.form)
            print(f"Resultado de procesar_update_perfil: {respuesta}")  # Depuración
            ...
            if respuesta == 1:
                ...
                flash('Error en la base de datos. Intente nuevamente.', 'error')
                return redirect(url_for('perfil'))

            # Si llega aquí, significa que no hubo un return válido antes
