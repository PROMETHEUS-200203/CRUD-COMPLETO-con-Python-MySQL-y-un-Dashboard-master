...
@app.route('/descargar-reporte-empleados')
def descargar_reporte_pdf():
    return generarReportePDF()

@app.route("/descargar-reporte-usuariosXls", methods=['GET'])
def descargar_reporte_usuarios_excel():
    if 'conectado' in session:
        # Llamar a la función que genera el archivo Excel
        return generarReporteUsuariosExcel()
    else:
        flash('Primero debes iniciar sesión.', 'error')
        return redirect(url_for('inicio'))

@app.route("/descargar-reporte-usuarios", methods=['GET'])
def descargar_reporte_usuarios_pdf():
    if 'conectado' in session:
        return generarReporteUsuariosPDF()
    else:
        flash('Primero debes iniciar sesión.', 'error')
        return redirect(url_for('inicio'))