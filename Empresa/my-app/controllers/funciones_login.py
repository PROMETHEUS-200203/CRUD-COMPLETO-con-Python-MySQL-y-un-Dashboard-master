...
def procesar_update_perfil(data_form):
    try:
        ...
        print(f"Datos recibidos en procesar_update_perfil: {data_form}")  # Depuración
                                ...
                                try:
                                    with connectionBD() as conexion_MySQLdb:
                                        with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                                            ...
                                            print("Filas afectadas:", cursor.rowcount)  # Depuración
                                            return cursor.rowcount or 4  # 4 -> No se hicieron cambios

                                except Exception as e:
                                    print(f"Error en update con contraseña: {e}")
                                    return 5  # Error en BD
                    else:
